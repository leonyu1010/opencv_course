...
void getEightBoundaryPoints(Size size, std::vector<Point2f> &boundaryPts)
{
...
void dlibLandmarksToPoints(dlib::full_object_detection &landmarks, std::vector<Point2f> &points)
{
...
void normalizeImagesAndLandmarks(Size outSize, Mat &imgIn, Mat &imgOut, std::vector<Point2f> &pointsIn,
...
void alignFace(Mat &imgIn, Mat &imgOut, dlib::rectangle faceRect, dlib::shape_predictor &landmarkDetector, Size outSize)
{
	int h = outSize.height;
	int w = outSize.width;

	std::vector<Point2f> pointsIn;

	dlib::cv_image<dlib::bgr_pixel> dlibIm(imgIn);
	dlib::full_object_detection landmarks = landmarkDetector(dlibIm, faceRect);
	dlibLandmarksToPoints(landmarks, pointsIn);
	...
	std::vector<Point2f> eyecornerDst;
	// Location of the left corner of left eye in normalized image.
	eyecornerDst.push_back(Point2f(0.2 * w, h / 3));
	// Location of the right corner of right eye in normalized image.
	eyecornerDst.push_back(Point2f(0.8 * w, h / 3));
	...
	imgIn.convertTo(imgIn, CV_8UC3, 255);
...
static void calculateDelaunayTriangles(Rect rect, std::vector<Point2f> &points,
...
std::vector<Point2f> getLandmarks(dlib::frontal_face_detector &faceDetector, dlib::shape_predictor &landmarkDetector,
								  Mat &img, float FACE_DOWNSAMPLE_RATIO = 1)
{
	...
	if (faceRects.size() > 0)
	{
		...
		dlib::rectangle scaledRect(
			(long)(rect.left() * FACE_DOWNSAMPLE_RATIO), (long)(rect.top() * FACE_DOWNSAMPLE_RATIO),
			(long)(rect.right() * FACE_DOWNSAMPLE_RATIO), (long)(rect.bottom() * FACE_DOWNSAMPLE_RATIO));
...
void warpImage(Mat &imgIn, Mat &imgOut, std::vector<Point2f> &pointsIn, std::vector<Point2f> &pointsOut,
			   std::vector<std::vector<int>> &delaunayTri)
{
