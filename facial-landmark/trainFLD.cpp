...
get_interocular_distances(const std::vector<std::vector<full_object_detection>> &objects, int numPoints)
{
...
int trainFLD(int argc, char **argv)
{
	try
	{
		...
		cout << "mean training error: "
			 << test_shape_predictor(sp, images_train, faces_train, get_interocular_distances(faces_train, numPoints))
			 << endl;

		// The real test is to see how well it does on data it wasn't trained
		// on.
		cout << "mean testing error:  "
			 << test_shape_predictor(sp, images_test, faces_test, get_interocular_distances(faces_test, numPoints))
			 << endl;
