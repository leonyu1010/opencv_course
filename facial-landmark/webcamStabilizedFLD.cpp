	...
	try
	{
		...
		while (1)
		{
			...
			for (unsigned long i = 0; i < faces.size(); ++i)
			{
				...
				rectangle r((long)(faces[i].left() * IMAGE_RESIZE), (long)(faces[i].top() * IMAGE_RESIZE),
							(long)(faces[i].right() * IMAGE_RESIZE), (long)(faces[i].bottom() * IMAGE_RESIZE));
				...
				cv::calcOpticalFlowPyrLK(imGrayPrevPyr, imGrayPyr, pointsPrev, points, status, err, winSize, maxLevel,
										 termcrit, 0, 0.0001);
			...
			cv::putText(im, cv::format("fps %.2f", fps), cv::Point(50, size.height - 50), cv::FONT_HERSHEY_COMPLEX, 1.5,
						cv::Scalar(0, 0, 255), 3);
		}
		cap.release();
