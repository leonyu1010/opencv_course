...
int camShift()
{
	...
	String faceCascadePath = DATA_PATH + "models/haarcascade_frontalface_default.xml";
	...
	while (1)
	{
		...
		RotatedRect rotatedWindow =
			CamShift(backProjectImage, currWindow, TermCriteria(TermCriteria::EPS | TermCriteria::COUNT, 10, 1));
		...
		rectangle(frameClone, Point(currWindow.x, currWindow.y),
				  Point(currWindow.x + currWindow.width, currWindow.y + currWindow.height), Scalar(255, 0, 0), 2,
				  LINE_AA);
		// Display the rotated rectangle with the orientation information
		for (int i = 0; i < 4; i++)
			line(frameClone, rotatedWindowVertices[i], rotatedWindowVertices[(i + 1) % 4], Scalar(0, 255, 0), 2,
				 LINE_AA);
